...
''')
       ...
       if len(meki) == 0:
          ...
          for memek in meki:
              try:
                  ...
                  open('data/login.txt','w').write(f'{memek}')
   ...
   def aset_ig(self):
       ...
       if os.path.isfile('data/login.txt') is True:
           self.coki = {'cookie':open('data/login.txt','r').read()}
       else:
           os.system('clear' if sys.platform.lower() == 'linux' else 'cls')
           self.Me()
       ...
       try:
           ...
           open('data/login.txt','w').write(f'{self.coki["cookie"]}')
       except:
 ...
 11. Cari Akun Verif Email   12. Crack Akun Facebook''',True)
   ...
   def chs(self, assets):
       while True:
         ...
         elif x in ['10']:os.system('rm -rf data/login.txt');self.insta()
         elif x in ['12']:
   ...
   def dumps(self, cintil, typess, xyz = []):
       if 'csrftoken' not in str(cintil['cookie']):
          try:
              ...
              os.system('rm -rf data/login.txt')
              exit(f'\n{P}[{K}!{P}] Csrftoken tidak tersedia, dump tidak akan berjalan: {e}')
       self.MyRich('[white]Masukan username akun instagram Pastikan (Target Memiliki Followers, Akun Bersifat Publik, Tidak Centang Biru) Ingin banyak target gunakan tanda koma sebagai pemisah contohnya [green](jhon62,smith_77,gustavo_)[/]',True)
